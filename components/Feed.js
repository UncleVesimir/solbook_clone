...
const Feed = ({ connected, name, url }) => {
  ...
  const getCommentsOnPost = async (index) => {
    ...
    try {
      let post = await program.account.postAccount.fetch(postSigner);
      ...
      console.log(program.account.postAccount.subscribe(postSigner));

      return comments.sort(
        (a, b) => a.commentTime.toNumber() - b.commentTime.toNumber()
      );
    } catch (error) {
      console.error(error);
    }
